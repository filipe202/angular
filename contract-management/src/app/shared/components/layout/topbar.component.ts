...
@Component({
  ...
  template: `
    <header class="header">
      <div class="search-bar">
        ...
        <input type="text" placeholder="Search contracts, parties, documents..." class="search-input">
      ...
      <div class="header-actions">
        ...
        <div class="avatar-area" (click)="logout()">
          <div class="avatar">{{ initials() }}</div>
          <div class="user-info">
            <span class="avatar-name">{{ userName() }}</span>
            <span class="avatar-role">Contract Manager</span>
          </div>
        </div>
      </div>
    </header>
  ...
  styles: [`
    ...
    .search-icon {
      ...
      color: var(--teal-500); pointer-events: none;
    }
    .search-input {
    ...
    .search-kbd {
      ...
      color: var(--teal-600); font-family: inherit;
    ...
    .avatar-role { font-size: 11px; color: var(--gray-400); margin-top: 2px; }
  `]
})
export class TopbarComponent {
  ...
  userName = computed(() => this.authService.user()?.name ?? 'John Mitchell');
  initials = computed(() => {
