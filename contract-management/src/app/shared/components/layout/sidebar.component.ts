...
@Component({
  ...
  styles: [`
    ...
    .logo-sub {
      ...
      color: var(--ch-teal); margin-top: 3px;
    }
    ...
    .nav-label {
      font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px;
      color: rgba(255,255,255,0.2); padding: 8px 10px 6px; margin-bottom: 2px;
    }
    .nav-item {
    ...
    .env-dot {
      ...
      background: var(--ch-teal); box-shadow: 0 0 6px rgba(13,148,136,0.5);
    }
  ...
  navItems = computed<NavItem[]>(() => {
    switch (this.authService.userRole()) {
      ...
      case Role.SIGNER: return [
        { label: 'Painel', icon: 'space_dashboard', route: '/signer/dashboard' },
